    ...
    while (source_count < size && dest_count < max) {
            ...
            if (iterations > 0) {
                ...
                if (found >= 1) {
                    ...
                    if (found) {
                        destination[dest_count++] = newch;
                        if (log_charconv) {
                            sprintf(logtmp, "%s(%02X[%c])~>%02X", sourcechars, (unsigned char) c, prch(c),
                                    (unsigned char) newch);
        ...
        if (found == 0) {
            ...
            if (found) {
                destination[dest_count++] = newch;
                if (log_charconv) {
                    sprintf(logtmp, "%02X[%c]~>%02X", (unsigned char) source[source_count], prch(source[source_count]),
                            (unsigned char) newch);
                    if (gsm2char(newch, &tmpch, 1))
                        sprintf(strchr(logtmp, 0), "[%c]", tmpch);
                    logch("%s ", logtmp);
        ...
        if (found == 0) {
            writelogfile0(LOG_NOTICE, 0,
                          tb_sprintf(
                                  "Cannot convert %i. character %c 0x%2X to GSM, you might need to update the translation tables.",
    ...
    while (source_count < size && dest_count < max) {
        if (source[source_count] != 0x1B) {
            ...
            if (gsm2char(source[source_count], &newch, 1))
                ...
                writelogfile0(LOG_NOTICE, 0,
                              tb_sprintf(
                                      "Cannot convert GSM character 0x%2X to ISO, you might need to update the 1st translation table.",
                                      source[source_count]));
#ifdef DEBUGMSG
                printf("%s\n", tb);
#endif
            ...
            if (gsm2char(source[source_count], &newch, 2))
                ...
                writelogfile0(LOG_NOTICE, 0,
                              tb_sprintf(
                                      "Cannot convert extended GSM character 0x1B 0x%2X, you might need to update the 2nd translation table.",
                                      source[source_count]));
#ifdef DEBUGMSG
                printf("%s\n", tb);
#endif
...
void logch(char *format, ...) {
    ...
    if (format) {
        ...
        if (strlen(logch_buffer) + strlen(text) < sizeof(logch_buffer)) {
            ...
            // Outgoing conversion:
            if (strlen(text) >= 3) if (strcmp(text + strlen(text) - 3, "20 ") == 0)
                flush = 1;
            // Incoming conversion:
            if (!flush) if (strlen(text) >= 6) if (strcmp(text + strlen(text) - 6, "20[ ] ") == 0)
                flush = 1;
            // Line wrap after a reasonable length reached:
            if (!flush) if (strlen(logch_buffer) > 80)
                flush = 1;
        }
#ifdef DEBUGMSG
        printf("%s", text);
...
int decode_7bit_packed(
    ...
    if ((septets % 8 == 0 && len && buffer[len - 1] == padding) ||
        (septets % 8 == 1 && len > 1 && buffer[len - 1] == padding && buffer[len - 2] == padding)) {
        len--;
#ifdef DEBUGMSG
