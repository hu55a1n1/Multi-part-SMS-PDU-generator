                ...
                for (x = 0; (size_t) x < strlen(command); x++) {
                    if (write(modem_handle, command + x, 1) < 1) {
                        if (print_error)
                            ...
                            writelogfile0(LOG_ERR, 1, tb_sprintf("Could not send character %c, cause: %s", command[x],
                                                                 strerror(errno)));
                            alarm_handler0(LOG_ERR, tb);
                        }
                        return 0;
...
#define SE   '\xF0' /* End sub negotiation */
        ...
        if (answer[idx] == IAC && idx + 1 < *len) {
            *response = 0;
            ...
            if (command == DO && got_option) {
                ...
                writelogfile(LOG_DEBUG, 0, "%s: Got WILL for %i (0x%02X), answering DONT.", title, (int) option,
                             option);
            }
            ...
            if (*response) if ((size_t) write(modem_handle, response, strlen(response)) != strlen(response))
    ...
    *response = 0;
    ...
    else if (DEVICE.telnet_password_prompt[0] && strstr(answer, DEVICE.telnet_password_prompt) &&
             DEVICE.telnet_password[0])
        ...
        if (got <= 0) {
            ...
            if (log_read_from_modem) {
                ...
                for (i = count; i < count + got; i++) {
                    ...
                    sprintf(strchr(tmp, 0), " %02X[%c]", (unsigned char) answer[i],
                            ((unsigned char) answer[i] >= ' ') ? answer[i] : '.');
                }
                        ...
                        if ((term = strchr(p, '\r'))) {
                            ...
                            if (!p2 || p2 > term) {
                                ...
                                if (!routed_pdu_store) {
                                    ...
                                    routed_pdu_store = (char *) realloc((void *) routed_pdu_store,
                                                                        strlen(routed_pdu_store) + pdu_length + 2);
                ...
                if (can_handle) {
                    if (is_sr && DEVICE.using_routed_status_report)
                        ...
                        writelogfile0(LOG_ERR, 0, tb_sprintf(
                                "Saved routed %s for later handling. However, you MUST DISABLE %s routing with modem settings.",
                                p, p));
                        alarm_handler0(LOG_ERR, tb);
                    }
                }
                    ...
                    writelogfile0(LOG_ERR, 0, tb_sprintf(
                            "Cannot handle this routed %s. You MUST DISABLE %s routing with modem settings.", p, p));
                    alarm_handler0(LOG_ERR, tb);
                }

                is_sr = 0;
                p = p1 + keyword_length;
                if (!(p1 = strstr(p, keyword))) if ((p1 = strstr(p, keyword_sr)))
                    is_sr = 1;
            }
        }
...
void do_hangup(char *answer) {
    ...
    if (DEVICE.hangup_incoming_call == 1 || (DEVICE.hangup_incoming_call == -1 && hangup_incoming_call == 1) ||
        DEVICE.phonecalls == 2) {
...
int put_command0(char *command, char *answer, int max, int timeout_count, char *expect, int silent) {
    ...
    if (DEVICE.communication_delay > 0) if (last_command_ended)
        ...
        *loganswer = 0;
        ...
        if (!i && *loganswer && DEVICE.detect_unexpected_input) {
            ...
            if (*loganswer) {
                ...
                if (!strstr(loganswer, "+CDSI:") && !strstr(loganswer, "+CMTI:")) if (!(strstr(loganswer, "+CLIP:") &&
                                                                                        DEVICE.phonecalls == 2)) if (!(
                        strstr(loganswer, "+CREG:") && get_loglevel() >= DEVICE.loglevel_lac_ci)) // 3.1.14.
                    writelogfile(LOG_ERR, DEVICE.unexpected_input_is_trouble, "Unexpected input: %s", loganswer);

                if (handlephonecall_clip(loganswer) != 1) if (strstr(loganswer, "RING") && DEVICE.phonecalls != 2)
                    do_hangup(loganswer);
            }
        }
    }
    ...
    if (!answer) {
        ...
        if (got_timeout) {
            ...
            if (expect && expect[0])
                writelogfile(LOG_DEBUG, 1, "put_command expected %s, timeout occurred. %i.", expect,
                             put_command_timeouts);
        }
        ...
        if (!got_timeout && strstr(answer, "ERROR")) {
            ...
            if (*p) if (max /*sizeof(answer)*/ - strlen(answer) > strlen(p) + 3)
...
int initmodem(char *new_smsc, int receiving) {
    ...
    if (DEVICE.pre_init > 0) {
        ...
        if (get_loglevel() >= DEVICE.loglevel_lac_ci) if (sizeof(command) > strlen(command) + 8)
    ...
    if (strcasecmp(DEVICE.pin, "ignore")) {
        ...
        // 3.1.12: Allow "echo on":
        if ((p = strstr(answer, "+CPIN:"))) if (p > answer)
            strcpyo(answer, p);
        ...
        if (!strstr(answer, "+CPIN: READY")) {
            ...
            writelogfile0(LOG_CRIT, 1, tb_sprintf("PIN handling: expected READY, modem answered %s%s%s",
                                                  change_crlf(cut_emptylines(cutspaces(answer)), ' '), (*p) ? ", " : "",
                                                  p));
            alarm_handler0(LOG_CRIT, tb);
            abnormal_termination(0);
        }
    ...
    if (DEVICE.initstring[0]) {
        ...
        do {
            ...
            if (strstr(answer, "ERROR")) if (retries < 2) if (t_sleep(errorsleeptime))
                return 7;
        }
    ...
    if (DEVICE.initstring2[0]) {
        ...
        if (!strstr(answer, "OK")) {
            ...
            writelogfile0(LOG_ERR, 1,
                          tb_sprintf("Modem did not accept the second init string%s%s", (*p) ? ", " : "", p));
            alarm_handler0(LOG_ERR, tb);
            return 3;
        }
            // 3.1.5:
    ...
    if (DEVICE.incoming && !reading_checked) {
        ...
        if (strstr(answer, "+CPMS: ,,,,,,,,")) {
            ...
            if (strstr(answer, "+CPMS: (),(),()")) {
                writelogfile0(LOG_ERR, 1,
                              tb_sprintf("Error: Looks like your device does not support reading of messages"));
                alarm_handler0(LOG_ERR, tb);
            }
        }
    }
    ...
    *netport = '\0';
    ...
    if (!(he = gethostbyname(hostname)))
        writelogfile(LOG_ERR, 0, "Couldn't resolve name '%s': %s.", hostname,
                     ...
                     (h_errno == NO_RECOVERY) ? "A non-recoverable name server error occured" : (h_errno == TRY_AGAIN)
                                                                                                ? "A temporary error occured."
...
int open_inet_socket(char *backend) {
    ...
    while (connect(fd, (struct sockaddr *) &sa, sizeof(sa)) != 0) {
        ...
        tb_sprintf("Couldn't connect socket %s, error: %s, waiting %i sec.", backend, strerror(errno),
                   DEVICE.socket_connection_errorsleeptime);
...
int openmodem() {
...
#endif
    {
        ...
        while ((modem_handle = open(DEVICE.device, O_RDWR | O_NOCTTY | O_NONBLOCK)) < 0) {
            ...
            tb_sprintf("Couldn't open serial port %s, error: %s, waiting %i sec.", DEVICE.device, strerror(errno),
                       DEVICE.device_open_errorsleeptime);
    ...
    if (modem_handle < 0) {
        if (modem_handle == -1) {
            writelogfile0(LOG_ERR, 1, tb_sprintf(
                    (DEVICE_IS_SOCKET) ? "Cannot open socket %s, error: %s" : "Cannot open serial port %s, error: %s",
    ...
    if (strstr(smsd_version, "beta")) {
        if (DEVICE_IS_SOCKET)
            ...
            writelogfile(LOG_INFO, 0, "Serial port %s opened as %i, rtscts: %i, baudrate: %i", DEVICE.device,
                         modem_handle, DEVICE.rtscts, DEVICE.baudrate);
    }
            ...
            if ((n = read(modem_handle, tmp, sizeof(tmp) - 1)) > 0) {
                ...
                if (log_read_from_modem) {
                    ...
                    for (i = 0; i < n; i++) {
                        ...
                        sprintf(strchr(temp, 0), " %02X[%c]", (unsigned char) answer[i],
                                ((unsigned char) answer[i] >= ' ') ? answer[i] : '.');
                    }
...
int wait_network_registration(int waitnetwork_errorsleeptime, int retry_count) {
    ...
    do {
        ...
        if ((p = strchr(answer, ','))) if ((p = strchr(p + 1, ',')))
            *p = 0;
        ...
        if (strstr(answer, ",1")) {
            ...
            if (retries >= DEVICE.log_not_registered_after) {
                writelogfile0(LOG_NOTICE, 1,
                              tb_sprintf("MODEM IS NOT REGISTERED, WAITING %i SEC. BEFORE RETRYING %i. TIME",
                                         waitnetwork_errorsleeptime, retries + 1));
            ...
            writelogfile0(LOG_ERR, 1, tb_sprintf(
                    "Error: Unexpected answer from Modem after +CREG?, waiting %i sec. before retrying",
                    waitnetwork_errorsleeptime));
            alarm_handler0(LOG_ERR, tb);
            if (t_sleep(waitnetwork_errorsleeptime))
                return -2;
        }
