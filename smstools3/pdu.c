...
void pdu_error(char **err_str, char *title, int position, int length, char *format, ...) {
    ...
    if (!(*err_str)) {
        ...
        *err_str = (char *) realloc((void *) *err_str,
                                    strlen(*err_str) + strlen(used_title) + strlen(tmp) + strlen(text) + 2);
    ...
    *tmp_smsc = 0;
    ...
    if (strcmp(mode, "old") == 0)
        ...
        if (*tmp_smsc)
            ...
            strcpy(pdu, "00");
        sprintf(strchr(pdu, 0), "%02X00%02X%02X%s%02X%02X%02X%02X", flags, numberlength, numberformat, tmp, proto,
    ...
    *udh_type = 0;
    ...
    while (idx < udh_length) {
        ...
        switch (i) {
            ...
            case 0x00:
                p = "Concatenated short messages, 8-bit reference number";
                break;
            case 0x01:
                p = "Special SMS Message Indication";
                break;
            case 0x02:
                p = "Reserved";
                break;
            ...
            case 0x04:
                p = "Application port addressing scheme, 8 bit address";
                break;
            case 0x05:
                p = "Application port addressing scheme, 16 bit address";
                break;
            case 0x06:
                p = "SMSC Control Parameters";
                break;
            case 0x07:
                p = "UDH Source Indicator";
                break;
            case 0x08:
                p = "Concatenated short message, 16-bit reference number";
                break;
            case 0x09:
                p = "Wireless Control Message Protocol";
                break;
            case 0x0A:
                p = "Text Formatting";
                break;
            case 0x0B:
                ...
                break;
            case 0x0C:
                p = "User Defined Sound (iMelody max 128 bytes)";
                break;
            case 0x0D:
                p = "Predefined Animation";
                break;
            case 0x0E:
                p = "Large Animation (16*16 times 4 = 32*4 =128 bytes)";
                break;
            case 0x0F:
                p = "Small Animation (8*8 times 4 = 8*4 =32 bytes)";
                break;
            case 0x10:
                p = "Large Picture (32*32 = 128 bytes)";
                break;
            case 0x11:
                p = "Small Picture (16*16 = 32 bytes)";
                break;
            case 0x12:
                p = "Variable Picture";
                break;
            case 0x13:
                p = "User prompt indicator";
                break;
            case 0x14:
                p = "Extended Object";
                break;
            case 0x15:
                p = "Reused Extended Object";
                break;
            case 0x16:
                p = "Compression Control";
                break;
            case 0x17:
                p = "Object Distribution Indicator";
                break;
            case 0x18:
                p = "Standard WVG object";
                break;
            case 0x19:
                p = "Character Size WVG object";
                break;
            case 0x1A:
                p = "Extended Object Data Request Command";
                break;
            case 0x20:
                p = "RFC 822 E-Mail Header";
                break;
            case 0x21:
                p = "Hyperlink format element";
                break;
            case 0x22:
                p = "Reply Address Element";
                break;
            case 0x23:
                p = "Enhanced Voice Mail Information";
                break;
        }
...
int pdu2text(char *pdu, char *text, int *text_length, int *expected_length,
    ...
    if (with_udh) {
        ...
        if (udh_type) if (explain_udh(udh_type, pdu + 2) < 0) if (strlen(udh_type) + 7 < SIZE_UDH_TYPE)
...
void explain_status(char *dest, size_t size_dest, int status) {
    ...
    switch (status) {
        case 0:
            p = "Ok,short message received by the SME";
            break;
        case 1:
            p = "Ok,short message forwarded by the SC to the SME but the SC is unable to confirm delivery";
            break;
        case 2:
            p = "Ok,short message replaced by the SC";
            break;

            // Temporary error, SC still trying to transfer SM
        case 32:
            p = "Still trying,congestion";
            break;
        case 33:
            p = "Still trying,SME busy";
            break;
        case 34:
            p = "Still trying,no response sendr SME";
            break;
        case 35:
            p = "Still trying,service rejected";
            break;
        case 36:
            p = "Still trying,quality of service not available";
            break;
        case 37:
            p = "Still trying,error in SME";
            break;
            // 38...47: Reserved
        ...
        case 64:
            p = "Error,remote procedure error";
            break;
        case 65:
            p = "Error,incompatible destination";
            break;
        case 66:
            p = "Error,connection rejected by SME";
            break;
        case 67:
            p = "Error,not obtainable";
            break;
        case 68:
            p = "Error,quality of service not available";
            break;
        case 69:
            p = "Error,no interworking available";
            break;
        case 70:
            p = "Error,SM validity period expired";
            break;
        case 71:
            p = "Error,SM deleted by originating SME";
            break;
        case 72:
            p = "Error,SM deleted by SC administration";
            break;
        case 73:
            p = "Error,SM does not exist";
            break;
            // 74...79: Reserved
        ...
        case 97:
            ...
            break;
        case 98:
            p = "Error,no response sendr SME";
            break;
        case 99:
            p = "Error,service rejected";
            break;
        case 100:
            p = "Error,quality of service not available";
            break;
        case 101:
            p = "Error,error in SME";
            break;
            // 102...105: Reserved
            // 106...111: Reserved
...
int split_type_0(char *full_pdu, char *Src_Pointer, int *alphabet, char *sendr, char *date, char *time,
    ...
    if (strlen(Src_Pointer) < 4)
        pdu_error(err_str, 0, Src_Pointer - full_pdu, 4, "While trying to read address length and address type: %s",
                  err_too_short);
    else {
        ...
        if (Length < 0 || Length > MAX_ADDRESS_LENGTH)
            ...
            if (i < 0)
                ...
                pdu_error(err_str, 0, Src_Pointer - full_pdu, 2, "Missing bit 7 in sender address type: \"%.2s\"",
                          Src_Pointer);
            else {
                Src_Pointer += 2;
                if ((i & 112) == 80) {  // Sender is alphanumeric
                    ...
                    if (strlen(Src_Pointer) < (size_t) (Length + padding))
                        ...
                        if (padding) {
                            if (sendr[i] != 'F')
                                add_warning(warning_headers,
                                            "Length of numeric sender address is odd, but not terminated with 'F'.");
                            else
                                sendr[i] = 0;
                        }
                        else {
                            if (sendr[i] == 'F') {
                                add_warning(warning_headers,
                                            "Length of numeric sender address is even, but still was terminated with 'F'.");
                                sendr[i] = 0;
                            }
                        }
        ...
        if (!(*err_str)) {
            ...
            if (strlen(Src_Pointer) < 20)
                pdu_error(err_str, 0, Src_Pointer - full_pdu, 20,
                          "While trying to read TP-PID, TP-DSC, TP-SCTS and TP-UDL: %s", err_too_short);
            else {
                if ((i = octet2bin_check(Src_Pointer)) < 0)
                    pdu_error(err_str, 0, Src_Pointer - full_pdu, 2, "Invalid protocol identifier: \"%.2s\"",
                              Src_Pointer);
                else {
                    if ((i & 0xF8) == 0x40)
                        *replace = (i & 0x07);
                    Src_Pointer += 2;
                    if ((i = octet2bin_check(Src_Pointer)) < 0)
                        pdu_error(err_str, 0, Src_Pointer - full_pdu, 2, "Invalid data encoding scheme: \"%.2s\"",
                                  Src_Pointer);
                    else {
                        *alphabet = (i & 0x0C) >> 2;
                        if (*alphabet == 3)
                            pdu_error(err_str, 0, Src_Pointer - full_pdu, 2,
                                      "Invalid alphabet in data encoding scheme: value 3 is not supported.");
                        ...
                        if (!(*err_str)) {
                            Src_Pointer += 2;
                            sprintf(date, "%c%c-%c%c-%c%c", Src_Pointer[1], Src_Pointer[0], Src_Pointer[3],
                                    Src_Pointer[2], Src_Pointer[5], Src_Pointer[4]);
                            if (!isdigitc(date[0]) || !isdigitc(date[1]) || !isdigitc(date[3]) || !isdigitc(date[4]) ||
                                !isdigitc(date[6]) || !isdigitc(date[7])) {
                                pdu_error(err_str, 0, Src_Pointer - full_pdu, 6,
                                          "Invalid character(s) in date of Service Centre Time Stamp: \"%s\"", date);
                                *date = 0;
                            }
                            ...
                            Src_Pointer += 6;
                            sprintf(time, "%c%c:%c%c:%c%c", Src_Pointer[1], Src_Pointer[0], Src_Pointer[3],
                                    Src_Pointer[2], Src_Pointer[5], Src_Pointer[4]);
                            if (!isdigitc(time[0]) || !isdigitc(time[1]) || !isdigitc(time[3]) || !isdigitc(time[4]) ||
                                !isdigitc(time[6]) || !isdigitc(time[7])) {
                                pdu_error(err_str, 0, Src_Pointer - full_pdu, 6,
                                          "Invalid character(s) in time of Service Centre Time Stamp: \"%s\"", time);
                                *time = 0;
                            }
                            ...
                            if (!(*err_str)) {
                                ...
                                if (octet2bin_check(Src_Pointer) < 0)
                                    pdu_error(err_str, 0, Src_Pointer - full_pdu, 2,
                                              "Invalid character(s) in Time Zone of Service Centre Time Stamp: \"%.2s\"",
                                              Src_Pointer);
                                else
                                    Src_Pointer += 2;
                            }
        ...
        if (!(*err_str)) {
            ...
            if (*alphabet <= 0) {
                if ((result = pdu2text(Src_Pointer, message, message_length, expected_length, with_udh, udh_data,
                                       udh_type, &errorpos)) < 0)
                ...
                if (bin_udh == 0) if (*alphabet == 1)
                    i = 0;
                if ((result = pdu2binary(Src_Pointer, message, message_length, expected_length, i, udh_data, udh_type,
                                         &errorpos)) < 0)
    ...
    if (strlen(Src_Pointer) < 6)
        ...
        if ((messageid = octet2bin_check(Src_Pointer)) < 0)
            ...
            if (Length < 1 || Length > MAX_ADDRESS_LENGTH)
                pdu_error(err_str, 0, Src_Pointer - full_pdu, 2, "Invalid recipient address length: \"%.2s\"",
                          Src_Pointer);
            else {
                padding = Length % 2;
                Src_Pointer += 2;
                i = explain_toa(from_toa, Src_Pointer, 0);
                if (i < 0)
                    pdu_error(err_str, 0, Src_Pointer - full_pdu, 2, "Invalid recipient address type: \"%.2s\"",
                              Src_Pointer);
                else if (i < 0x80)
                    pdu_error(err_str, 0, Src_Pointer - full_pdu, 2,
                              "Missing bit 7 in recipient address type: \"%.2s\"", Src_Pointer);
                else {
                    Src_Pointer += 2;
                    if ((i & 112) == 80) {  // Sender is alphanumeric
                        if (strlen(Src_Pointer) < (size_t) (Length + padding))
                        ...
                        if (strlen(Src_Pointer) < (size_t) (Length + padding))
                            ...
                            if (padding) {
                                if (sendr[i] != 'F')
                                    add_warning(warning_headers,
                                                "Length of numeric recipient address is odd, but not terminated with 'F'.");
                                else
                                    sendr[i] = 0;
                            }
                            else {
                                if (sendr[i] == 'F') {
                                    add_warning(warning_headers,
                                                "Length of numeric recipient address is even, but still was terminated with 'F'.");
                                    sendr[i] = 0;
                                }
                            }
                    ...
                    if (!(*err_str)) {
                        ...
                        if (strlen(Src_Pointer) < 14)
                            pdu_error(err_str, 0, Src_Pointer - full_pdu, 14, "While trying to read SMSC Timestamp: %s",
                                      err_too_short);
                        else {
                            // get SMSC timestamp
                            sprintf(date, "%c%c-%c%c-%c%c", Src_Pointer[1], Src_Pointer[0], Src_Pointer[3],
                            ...
                            if (!isdigitc(date[0]) || !isdigitc(date[1]) || !isdigitc(date[3]) || !isdigitc(date[4]) ||
                                !isdigitc(date[6]) || !isdigitc(date[7])) {
                                pdu_error(err_str, 0, Src_Pointer - full_pdu, 6,
                                          "Invalid character(s) in date of SMSC Timestamp: \"%s\"", date);
                                *date = 0;
                            }
                            else if (atoi(date + 3) > 12 || atoi(date + 6) > 31) {
                                // Not a fatal error (?)
                            ...
                            if (!isdigitc(time[0]) || !isdigitc(time[1]) || !isdigitc(time[3]) || !isdigitc(time[4]) ||
                                !isdigitc(time[6]) || !isdigitc(time[7])) {
                                pdu_error(err_str, 0, Src_Pointer - full_pdu, 6,
                                          "Invalid character(s) in time of SMSC Timestamp: \"%s\"", time);
                                *time = 0;
                            }
                            else if (atoi(time) > 23 || atoi(time + 3) > 59 || atoi(time + 6) > 59) {
                                // Not a fatal error (?)
                            ...
                            if (!(*err_str)) {
                                ...
                                if (octet2bin_check(Src_Pointer) < 0)
                                    pdu_error(err_str, 0, Src_Pointer - full_pdu, 2,
                                              "Invalid character(s) in Time Zone of SMSC Time Stamp: \"%.2s\"",
                                              Src_Pointer);
                                else
                                    Src_Pointer += 2;
                            }
                    ...
                    if (!(*err_str)) {
                        if (strlen(Src_Pointer) < 14)
                            pdu_error(err_str, 0, Src_Pointer - full_pdu, 14,
                                      "While trying to read Discharge Timestamp: %s", err_too_short);
                        else {
                            // get Discharge timestamp
                            sprintf(temp, "%c%c-%c%c-%c%c %c%c:%c%c:%c%c", Src_Pointer[1], Src_Pointer[0],
                                    Src_Pointer[3], Src_Pointer[2], Src_Pointer[5], Src_Pointer[4], Src_Pointer[7],
                                    Src_Pointer[6], Src_Pointer[9], Src_Pointer[8], Src_Pointer[11], Src_Pointer[10]);
                            if (!isdigitc(temp[0]) || !isdigitc(temp[1]) || !isdigitc(temp[3]) || !isdigitc(temp[4]) ||
                                !isdigitc(temp[6]) || !isdigitc(temp[7]) ||
                                !isdigitc(temp[9]) || !isdigitc(temp[10]) || !isdigitc(temp[12]) ||
                                !isdigitc(temp[13]) || !isdigitc(temp[15]) || !isdigitc(temp[16]))
                                pdu_error(err_str, 0, Src_Pointer - full_pdu, 12,
                                          "Invalid character(s) in Discharge Timestamp: \"%s\"", temp);
                            else if (atoi(temp + 3) > 12 || atoi(temp + 6) > 31 || atoi(temp + 9) > 24 ||
                                     atoi(temp + 12) > 59 || atoi(temp + 16) > 59) {
                                // Not a fatal error (?)
                            ...
                            if (!(*err_str)) {
                                ...
                                if (octet2bin_check(Src_Pointer) < 0)
                                    pdu_error(err_str, 0, Src_Pointer - full_pdu, 2,
                                              "Invalid character(s) in Time Zone of Discharge Time Stamp: \"%.2s\"",
                                              Src_Pointer);
                                else
                                    Src_Pointer += 2;
                            }
                        ...
                        if (!(*err_str)) {
                            ...
                            if (strlen(Src_Pointer) < 2)
                                pdu_error(err_str, 0, Src_Pointer - full_pdu, 2,
                                          "While trying to read Status octet: %s", err_too_short);
                            else {
                                // get Status
                                if ((status = octet2bin_check(Src_Pointer)) < 0)
                                    pdu_error(err_str, 0, Src_Pointer - full_pdu, 2, "Invalid Status octet: \"%.2s\"",
                                              Src_Pointer);
                                else {
                                    char buffer[128];
...
int splitpdu(char *pdu, char *mode, int *alphabet, char *sendr, char *date, char *time, char *message,
    ...
    int message_length;
    int expected_length;
    char tmp_udh_data[SIZE_UDH_DATA] = {0};
    char tmp_udh_type[SIZE_UDH_TYPE] = {0};
    char *udh_data;
    char *udh_type;
        ...
        *flash = 0;
        ...
        if (strlen(Pointer) < 2)
                ...
                if ((Length = octet2bin_check(Pointer)) < 0)
                    ...
                    if (Length == 0)
                        ...
                        if (Length < 2 || Length > MAX_SMSC_ADDRESS_LENGTH)
                            pdu_error(&err_str, 0, Pointer - pdu, 2, "Invalid sender SMSC address length: \"%.2s\"",
                                      Pointer);
                        else {
                            Length = Length * 2 - 2;
                            // No padding because the given value is number of octets.
                            if (strlen(Pointer) < (size_t) (Length + 4))
                                pdu_error(&err_str, 0, Pointer - pdu, Length + 4,
                                          "While trying to read sender SMSC address (length %i): %s",
                                          Length, err_too_short);
                            else {
                                Pointer += 2;
                                i = octet2bin_check(Pointer);
                                if (i < 0)
                                    pdu_error(&err_str, 0, Pointer - pdu, 2,
                                              "Invalid sender SMSC address type: \"%.2s\"", Pointer);
                                else if (i < 0x80)
                                    pdu_error(&err_str, 0, Pointer - pdu, 2,
                                              "Missing bit 7 in sender SMSC address type: \"%.2s\"", Pointer);
                                else {
                                    Pointer += 2;
                                    ...
                                    if ((i & 112) == 80) {
                                        ...
                                        for (i = 0; smsc[i]; i++)
                                            if (!isXdigit(smsc[i])) {
                                                pdu_error(&err_str, 0, Pointer - pdu, Length,
                                                          "Invalid character(s) in alphanumeric SMSC address: \"%s\"",
                                                          smsc);
                                                *smsc = 0;
                                                break;
                                            }
                                        ...
                                        for (i = 0; smsc[i]; i++)
                                            if (!isdigitc(smsc[i])) {
                                                ...
                                                // *smsc = 0;
                                                add_warning(warning_headers,
                                                            "Invalid character(s) in numeric SMSC address");
                                                break;
                                            }
                                    }
            ...
            if (!err_str) {
                if (strlen(Pointer) < 2)
                    pdu_error(&err_str, 0, Pointer - pdu, 2,
                              "While trying to read First octet of the SMS-DELIVER PDU: %s", err_too_short);
                else {
                    if ((i = octet2bin_check(Pointer)) < 0)
                        pdu_error(&err_str, 0, Pointer - pdu, 2, "While reading First octet of the SMS-DELIVER PDU: %s",
                                  err_pdu_content);
                    else {
                        // Unused bits 3 and 4 should be zero, failure with this produces a warning:
                        if (i & 0x18)
                            add_warning(warning_headers,
                                        "Unused bits 3 and 4 are used in the first octet of the SMS-DELIVER PDU.");
                        ...
                        {
                            Pointer += 2;
                            result = split_type_0(pdu, Pointer, alphabet, sendr, date, time, message, &message_length,
                                                  &expected_length,
                                                  *with_udh, udh_data, udh_type, from_toa, replace, &err_str,
                                                  warning_headers, flash, bin_udh);
                        ...
                        {
                            Pointer += 2;
                            result = split_type_2(pdu, Pointer, sendr, date, time, message, from_toa, &err_str,
                                                  warning_headers);
                            *is_statusreport = 1;
                        }
                        else if (Type == 1) // Sent message
                        {
                            ...
                            *is_unsupported_pdu = 1;
                            ...
                            pdu_error(&err_str, "", -1, 0, "%s%.2s%s%i%s", "The PDU data (", Pointer,
                                      ") says that the message format is ",
...
int get_pdu_details(char *dest, size_t size_dest, char *pdu, int mnumber) {
    ...
    udlen = splitpdu(pdu, DEVICE.mode, &alphabet, sender, date, time, ascii, smsc, &with_udh, udh_data, udh_type,
                     &is_statusreport, &is_unsupported_pdu, from_toa, &report, &replace, warning_headers, &flash,
                     bin_udh);
                                                        ...
                                                        m_id
                                                            p_number
                                                                p_count
                                                                    incoming / report
*/
            snprintf(buffer, sizeof(buffer), "%.03i %c %-8.8s %-8.8s %s%.03i %.03i/%.03i %c\n", mnumber, sort_ch, date,
                     time, sender, m_id, p_number, p_count, (is_statusreport) ? 'r' : 'i');
