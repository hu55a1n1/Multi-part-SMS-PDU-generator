...
int movefilewithdestlock_new(char *filename, char *directory, int keep_fname, int store_original_fname, char *prefix,
                             char *newfilename) {
    if (newfilename)
        *newfilename = 0;
...
int getfile(int trust_directory, char *dir, char *filename, int lock) {
    ...
    while (1) {
        ...
        while ((ent = readdir(dirdata))) {
            ...
            // 3.1beta7, 3.0.10:
            if (os_cygwin) if (!check_access(tmpname))
                chmod(tmpname, 0766);
            ...
            if (!trust_directory && !file_is_writable(tmpname)) {
                ...
                if (!check_access(tmpname)) {
                    ...
                    if (getfile_err_store) if (strstr(getfile_err_store, storage_key))
                        report = 0;
                ...
                if (found && statbuf.st_mtime == mtime) if (strcmp(fname, tmpname) > 0)
                    mtime = 0;
            ...
            if (stat(fname, &statbuf) || !lockfile(fname)) {
                ...
                if (found == 0) {
                    ...
                    if (max_continuous_sending == 0 || time_usec() < start_time + max_continuous_sending * 1000000) {
                        ...
                        writelogfile(LOG_DEBUG, 0,
                                     "Tried to get a file for %i seconds, will do other tasks and then continue.",
                                     (int) (time_usec() - start_time) / 1000000);
                }
            }
        }
    ...
    if (!found)
        *filename = 0;
        ...
        if (i > 10)
            writelogfile((i >= 50) ? LOG_NOTICE : LOG_DEBUG, 0,
                         "Took %.1f seconds to get a file %s, lost %i times, %i SMS files and %i LOCK files seen.",
                         (double) i / 10, fname, lost_count, files_count, locked_count);
    }
...
int my_system(
    ...
    if (pid == 0)        // only executed in the child
    {
        ...
        writelogfile0(LOG_CRIT, 1,
                      tb_sprintf("Fatal error: execv( %s ) returned: %i, %s", shell, errno, strerror(errno)));
    ...
    while (1) {
        if (waitpid(pid, &status, 0) == -1) {
            if (errno != EINTR) {
                *run_info = 0;
                writelogfile0(LOG_ERR, 0,
                              tb_sprintf("Done: %s, execution time %i sec., errno: %i, %s", info, time(0) - start_time,
            ...
            *run_info = 0;
            ...
            writelogfile0(level, trouble,
                          tb_sprintf("Done: %s, execution time %i sec., status: %i (%i)", info, time(0) - start_time,
            ...
            if (!ignore_exec_output) {
                ...
                for (i = 1; i <= 2; i++) {
                    ...
                    if (stat(p, &statbuf) == 0) {
                        if (statbuf.st_size) {
                            writelogfile0(LOG_ERR, 1, tb_sprintf("Exec: %s %s:", info,
                                                                 (i == 1) ? "said something" : "encountered errors"));
                            if ((fp = fopen(p, "r"))) {
...
int check_pid(char *filename) {
    ...
    sprintf(pid, "%i\n", (int) getpid());
    if ((fp = fopen(filename, "r"))) {
        if (fgets(buffer, sizeof(buffer), fp)) if (strcmp(pid, buffer))
            result = atoi(buffer);
