...
char HDR_System_message2[SIZE_HEADER] = {0};
const char *HDR_Smsd_debug = "Smsd_debug:";    // For debugging purposes
char HDR_Smsd_debug2[SIZE_HEADER] = {0};
...
int exec_system(char *command, char *info) {
    ...
    else if (!strcmp(info, EXEC_RR_MODEM))
        i = &last_status; //_rr_modem;
    else if (!strcmp(info, EXEC_RR_POST_MODEM))
        i = &last_status; //_rr_post_modem;
    else if (!strcmp(info, EXEC_RR_MAINPROCESS))
            ...
            if (*i <= 0) {
                ...
                if (to) {
                    ...
                    if (*adminmessage_device && process_id == -1 && shared_buffer) {
                        ...
                        if (*shared_buffer) {
                            timeout = 1;
                            writelogfile(LOG_INFO, 1, "Timeout while trying to deliver alert to %s",
                                         adminmessage_device);
                        }
                        else {
                            snprintf(shared_buffer, SIZE_SHARED_BUFFER, "%s Sms3: mainprocess, %s", adminmessage_device,
                                     alert);
                            sendsignal2devices(SIGCONT);
                        }
                    }
            ...
            while ((getline_result = my_getline(fp, name, sizeof(name), value, sizeof(value))) != 0) {
                ...
                if (*value == '-') {
                    ...
                    if (!strcasecmp(name, HDR_From) ||
                        ...
                        fprintf(stderr, "%s\n",
                                tb_sprintf("In language file, translation for %s cannot start with '-' character",
                                           name));
                        writelogfile(LOG_CRIT, 1, "%s", tb);
                        result++;
                        continue;
                ...
                if (!strcasecmp(name, "incoming"))
                    ...
                    if (!strchr(value, '%') ||
                        ...
                        fprintf(stderr, "%s\n",
                                tb_sprintf("In language file, format for datetime is completelly wrong: \"%s\"",
                                           value));
                        writelogfile(LOG_CRIT, 1, "%s", tb);
                        result++;
                        continue;
                    ...
                    if ((i = strlen(value)) > 0) {
                        if (value[i - 1] != '\'') if (i < SIZE_HEADER - 1)
                            strcat(value, "'");
                    }
                    else {
        ...
        if (!(fp = fopen(filename, "r"))) {
            ...
            if (rename(tmp_filename, filename) != 0) {
                if (!(fptmp = fopen(tmp_filename, "r"))) {
                    writelogfile0(LOG_WARNING, 1,
                                  tb_sprintf("Header handling aborted, reading %s failed", tmp_filename));
                    alarm_handler0(LOG_WARNING, tb);
                    result = 2;
                }
                else {
                    if (!(fp = fopen(filename, "w"))) {
                        writelogfile0(LOG_WARNING, 1,
                                      tb_sprintf("Header handling aborted, creating %s failed", filename));
                        alarm_handler0(LOG_WARNING, tb);
                        result = 1;
                    }
                    else {
                   ...
                   int *to_type)        /* -1 = default, -2 = error, >= 0 = accepted value  */
{
    ...
    if ((File = fopen(filename, "r"))) {
        ...
        while (fgets(line, sizeof(line), File)) {
            ...
            else if (test_header(&hlen, line, HDR_ToTOA, HDR_ToTOA2)) {
            ...
            else if (test_header(&hlen, line, HDR_From, HDR_From2)) {
                if (strlen(cutspaces(strcpyo(line, line + hlen))) < SIZE_FROM)
                    strcpy(from, line);
            }
            else if (test_header(&hlen, line, HDR_SMSC, HDR_SMSC2)) {
            ...
            else if (test_header(&hlen, line, HDR_Flash, HDR_Flash2))
                *flash = yesno(cutspaces(strcpyo(line, line + hlen)));
            else if (test_header(&hlen, line, HDR_Provider, HDR_Provider2)) {
                if (strlen(cutspaces(strcpyo(line, line + hlen))) < SIZE_QUEUENAME)
                    strcpy(queue, line);
            }
            else if (test_header(&hlen, line, HDR_Queue, HDR_Queue2)) {
                if (strlen(cutspaces(strcpyo(line, line + hlen))) < SIZE_QUEUENAME)
                    strcpy(queue, line);
            }
            else if (test_header(&hlen, line, HDR_Binary, HDR_Binary2))
                *alphabet = yesno(cutspaces(strcpyo(line, line + hlen)));
            else if (test_header(&hlen, line, HDR_Report, HDR_Report2))
                *report = yesno(cutspaces(strcpyo(line, line + hlen)));
            else if (test_header(&hlen, line, HDR_Autosplit, HDR_Autosplit2))
                *split = atoi(cutspaces(strcpyo(line, line + hlen)));
            else if (test_header(&hlen, line, HDR_Validity, HDR_Validity2))
                *validity = parse_validity(cutspaces(strcpyo(line, line + hlen)), *validity);
            else if (test_header(&hlen, line, HDR_Voicecall, HDR_Voicecall2))
                *voicecall = yesno(cutspaces(strcpyo(line, line + hlen)));
            else if (test_header(&hlen, line, HDR_Hex, HDR_Hex2))
                *hex = yesno(cutspaces(strcpyo(line, line + hlen)));
            else if (test_header(&hlen, line, HDR_Replace, HDR_Replace2)) {
                *replace_msg = atoi(cutspaces(strcpyo(line, line + hlen)));
                if (*replace_msg < 0 || *replace_msg > 7)
                    *replace_msg = 0;
            }
            else if (test_header(&hlen, line, HDR_Alphabet, HDR_Alphabet2)) {
                cutspaces(strcpyo(line, line + hlen));
            ...
            else if (strncmp(line, HDR_UDHDATA, hlen = strlen(HDR_UDHDATA)) == 0) {
                if (strlen(cutspaces(strcpyo(line, line + hlen))) < SIZE_UDH_DATA)
                    strcpy(udh_data, line);
            }
            else if (strncmp(line, HDR_UDHDUMP, hlen = strlen(HDR_UDHDUMP)) ==
                     0) // same as UDH-DATA for backward compatibility
            {
                if (strlen(cutspaces(strcpyo(line, line + hlen))) < SIZE_UDH_DATA)
                    strcpy(udh_data, line);
            }
            else if (strncmp(line, HDR_UDH, hlen = strlen(HDR_UDH)) == 0)
                *with_udh = yesno(cutspaces(strcpyo(line, line + hlen)));
            else if (test_header(&hlen, line, HDR_Include, HDR_Include2)) {
                cutspaces(strcpyo(line, line + hlen));
                if (recursion_level < 1)
                    readSMSheader(line, recursion_level + 1, to, from, alphabet, with_udh, udh_data, queue, flash,
                                  smsc, report, split, validity, voicecall, hex, replace_msg, macros, system_msg,
                                  to_type);
            }
            else if (test_header(&hlen, line, HDR_Macro, HDR_Macro2)) {
                char *p, *p1, *p2;
            ...
            else if (test_header(&hlen, line, HDR_System_message, HDR_System_message2)) {
                // 3.1.7:
            ...
            else if (test_header(&hlen, line, HDR_Smsd_debug, HDR_Smsd_debug2)) {
...
void readSMShex(char *filename, int recursion_level, char *text, int *textlen, char *macros, char *errortext) {
        ...
        while (fgets(line, sizeof(line), fp) && !too_long) {
            ...
            if (strncmp(line, "INLINESTRING:", 13) == 0) {
                ...
                *textlen += strlen(line) - 7;
                ...
                if (*line) {
                    if (strlen(line) % 2 != 0) {
                        writelogfile0(LOG_ERR, 1, tb_sprintf(
                                "Hex presentation error in sms file %s: incorrect length of data: \"%s\".", filename,
                                line));
                        alarm_handler0(LOG_ERR, tb);
                        if (errortext)
                            strcpy(errortext, tb);
                    ...
                    if (i < 1) {
                        writelogfile0(LOG_ERR, 1,
                                      tb_sprintf("Hex conversion error in sms file %s: \"%s\".", filename, p));
                        alarm_handler0(LOG_ERR, tb);
                        if (errortext)
                            strcpy(errortext, tb);
                        text[0] = 0;
    ...
    if (errortext) if ((p = strstr(errortext, filename)))
    ...
    while (terminate == 0) {
        ...
        if ((now >= last_spooling + delaytime_mainprocess) || break_workless_delay) {
            ...
            if ((i = check_pid(pidfile))) {
                writelogfile(LOG_ERR, 1,
                             tb_sprintf("FATAL ERROR: Looks like another smsd (%i) is running. I (%i) quit now.", i,
                                        (int) getpid()));
            ...
            *tb = 0;
            ...
            if (getfile(trust_outgoing, d_spool, filename, 0)) {
                ...
                if (i == 2) {
                    ...
                    readSMSheader(filename, 0, to, from, &alphabet, &with_udh, udh_data, queuename, &flash, smsc,
                                  &report, &split,
                    ...
                    if (to[0] == 0) {
                        ...
                        writelogfile(LOG_NOTICE, 0,
                                     tb_sprintf("Destination %s in file %s is blacklisted", to, filename));
                        alarm_handler(LOG_NOTICE, tb);
                        ...
                        writelogfile(LOG_NOTICE, 0,
                                     tb_sprintf("Destination %s in file %s is not whitelisted", to, filename));
                        alarm_handler(LOG_NOTICE, tb);
...
#endif
                    {
                        ...
                        writelogfile(LOG_NOTICE, 0,
                                     tb_sprintf("Wrong provider queue %s in file %s", queuename, filename));
                        alarm_handler(LOG_NOTICE, tb);
                        ...
                        writelogfile(LOG_NOTICE, 0,
                                     tb_sprintf("Destination number %s in file %s does not match any provider", to,
                                                filename));
                        alarm_handler(LOG_NOTICE, tb);
                        ...
                        if (movefilewithdestlock_new(filename, directory, keep_filename, store_original_filename, "",
                                                     newfilename) == 1) {
                            // 1 = lockfile cannot be created. It exists.
                            writelogfile0(LOG_CRIT, 1,
                                          tb_sprintf("Conflict with .LOCK file in the spooler: %s %s", filename,
                                                     directory));
                            alarm_handler0(LOG_CRIT, tb);
                            ...
                            if (movefilewithdestlock_new(filename, directory, keep_filename, store_original_filename,
                                ...
                                writelogfile0(LOG_CRIT, 0,
                                              tb_sprintf("Retry solved the spooler conflict: %s %s", filename,
                                                         directory));
                        }

                        stop_if_file_exists("Cannot move", filename, "to", directory);
                        writelogfile(LOG_NOTICE, 0, "Moved file %s to %s", filename,
                                     (keep_filename) ? directory : newfilename);
                        success = 1;
                        sendsignal2devices(SIGCONT);
                    }
                }
                ...
                if (!success) {
                    ...
                    if (stat(filename, &statbuf) == 0) if (statbuf.st_size == 0)
                        file_is_empty = 1;
                    ...
                    *add_headers = 0;
                    ...
                    if (d_failed[0] && !file_is_empty) {
                        movefilewithdestlock_new(filename, d_failed, keep_filename, store_original_filename,
                                                 process_title, newfilename);
                        stop_if_file_exists("Cannot move", filename, "to", d_failed);
                    }

                    if (eventhandler[0]) {
                        snprintf(cmdline, sizeof(cmdline), "%s %s %s", eventhandler, "FAILED",
                                 (d_failed[0] == 0 || file_is_empty) ? filename : newfilename);
...
int check_memory(int *used_memory, int *max_memory, char *memory_list, size_t memory_list_size,
    ...
    *answer = 0;
    ...
    else if (value_in(DEVICE.check_memory_method, 5, CM_CMGL, CM_CMGL_DEL_LAST, CM_CMGL_CHECK, CM_CMGL_DEL_LAST_CHECK,
                      CM_CMGL_SIMCOM)) {
            ...
            while (*pos) {
                ...
                if (i <= p_count) {
                    ...
                    if (ms_purge > 0) {
                        ...
                        writelogfile0(LOG_INFO, 0, tb_sprintf("Skipping message %s from %s, all parts not found",
                                                              sender_id + offset_messageid, sender));
                        if (remaining > 0)
                            writelogfile0(LOG_DEBUG, 0, tb_sprintf(
                                    "Message %s from %s will be %s after %i minutes unless remaining parts are received",
                                    sender_id + offset_messageid, sender,
                                    (DEVICE.ms_purge_read) ? "read partially" : "deleted", remaining));
                    }
                }
...
int savephonecall(char *entry_number, int entry_type, char *entry_text) {
    ...
    if (!(fp = fopen(filename, "w"))) {
        writelogfile0(LOG_ERR, 1,
                      tb_sprintf("Could not create file %s for phonecall from %s: %s", filename, entry_number,
                                 strerror(errno)));
        alarm_handler0(LOG_ERR, tb);
        result = 1;
    }
        ...
        if (DEVICE.number[0]) if (*HDR_Number2 != '-')
        ...
        if (eventhandler[0] || DEVICE.eventhandler[0]) {
            snprintf(cmdline, sizeof(cmdline), "%s %s %s",
                     (DEVICE.eventhandler[0]) ? DEVICE.eventhandler : eventhandler, "CALL", filename);
            exec_system(cmdline, EXEC_EVENTHANDLER);
        }
    }
...
int received2file(char *line1, char *line2, char *filename, int *stored_concatenated, int incomplete) {
    ...
    char sendr[100] = {0};
    int with_udh = 0;
    char udh_data[SIZE_UDH_DATA] = {0};
    char udh_type[SIZE_UDH_TYPE] = {0};
    char smsc[31] = {0};
    char name[64] = {0};
    char date[9] = {0};
    char Time[9] = {0};
    char warning_headers[SIZE_WARNING_HEADERS] = {0};
    ...
    userdatalength = splitpdu(line2, DEVICE.mode, &alphabet, sendr, date, Time, ascii, smsc, &with_udh, udh_data,
                              udh_type,
                              &is_statusreport, &is_unsupported_pdu, from_toa, &report, &replace, warning_headers,
                    ...
                    if (p_count == 1 && p_number == 1) {
                        ...
                        if (!(*udh_data)) {
                            ...
                            *udh_type = 0;
                            ...
                            if (explain_udh(udh_type, udh_data) < 0) if (strlen(udh_type) + 7 < SIZE_UDH_TYPE)
  ...
  printf("!! replace=%i\n", replace);
    ...
    *stored_concatenated = 0;
    if (do_internal_combine == 1) {
        ...
        int m_id;
        char storage_udh[SIZE_UDH_DATA] = {0};
        int a_type;
  ...
  printf("!!\n");
                ...
                if (!found) {
                        ...
                        while (fgets(st, sizeof(st), fd)) {
                            ...
                            if (strncmp(st + 9, messageid, strlen(messageid)) == 0 && part == i &&
                                ...
                                if (i == 1) // udh_data and _type are taken from the first part only.
                                    udlen = splitpdu(p, DEVICE.mode, &alphabet, sendr, date, Time,
                                                     ascii + userdatalength, smsc,
                                                     &with_udh, udh_data, udh_type, &is_statusreport,
                                                     &is_unsupported_pdu,
                                                     from_toa, &report, &replace, warning_headers, &flash,
                                                     do_internal_combine_binary);
                                else
                                    udlen = splitpdu(p, DEVICE.mode, &alphabet, sendr, date, Time,
                                                     ascii + userdatalength, smsc,
                                ...
                                if (alphabet == -1 && DEVICE.cs_convert == 1)
                                    udlen = gsm2iso(ascii + userdatalength, udlen, ascii + userdatalength,
                  ...
                  alphabet = 4;
                    ...
                    if (stat(con_filename, &statbuf) == 0) if (statbuf.st_size == 0)
                        unlink(con_filename);
    ...
    if (result) {
        if (*stored_concatenated)
            ...
            make_datetime_string(timestamp, sizeof(timestamp), 0, 0, date_filename_format);
            if (date_filename == 1)
                sprintf(filename, "%s/%s.%s.XXXXXX", (is_statusreport && *d_report) ? d_report : d_incoming, timestamp,
                        DEVICE.name);
            else if (date_filename == 2)
                sprintf(filename, "%s/%s.%s.XXXXXX", (is_statusreport && *d_report) ? d_report : d_incoming,
            ...
            if ((fd = fopen(filename, "w"))) {
                ...
                // 3.1beta7: new header:
                if (*HDR_Length2 != '-')
                    fprintf(fd, "%s %i\n", get_header_incoming(HDR_Length, HDR_Length2),
                            (alphabet == 2) ? userdatalength / 2 : userdatalength);
                ...
                if (store_received_pdu >= 1 && is_unsupported_pdu == 1) {
                    ...
                    if ((p = strstr(ascii, "Position "))) {
                        if ((pos = atoi(p + 9)) > 0) {
                            if ((p = strchr(p + 9, ','))) if ((p2 = strchr(p, ':')) && p2 > p + 1)
                                len = atoi(p + 1);
                            fprintf(fd, "Pos: ");
                            while (i++ < pos - 1)
                                if (i % 10) {
                                                                                                                                        ...
                                                                                                                                        if ((fd = fopen(filename, "r")))
  {
    ...
    fclose(fd);
                ...
                writelogfile(LOG_INFO, 0, "Wrote an incoming %smessage file: %s", (incomplete) ? "incomplete " : "",
                             filename);
...
// Returns -1 on error
{
    ...
    for (memories = 0; memories <= 2; memories++) {
        ...
        *check_memory_buffer = 0;
        ...
        if (check_memory(&used_memory, &max_memory, memory_list, sizeof(memory_list), delete_list,
                         sizeof(delete_list)) == 0)
            break;
        ...
        if (used_memory > 0) {
            ...
            if (!value_in(DEVICE.check_memory_method, 6, CM_CMGD, CM_CMGL, CM_CMGL_DEL_LAST, CM_CMGL_CHECK,
                          CM_CMGL_DEL_LAST_CHECK, CM_CMGL_SIMCOM))
                sim = DEVICE.read_memory_start;

            for (; ;) {
                if (!(*delete_list)) if (terminate == 1)
                    break;
                    ...
                    *quick = 1;
                    ...
                    if (stored_concatenated == 0) {
                        if (eventhandler[0] || DEVICE.eventhandler[0]) {
                            ...
                            snprintf(cmdline, sizeof(cmdline), "%s %s %s", handler,
                                     (statusreport) ? "REPORT" : "RECEIVED", filename);
                            exec_system(cmdline, EXEC_EVENTHANDLER);
                        }
                    }
                    ...
                    else if (!value_in(DEVICE.check_memory_method, 1, CM_CMGL_SIMCOM))
                        deletesms(found);
...
int send_part(char *from, char *to, char *text, int textlen, int alphabet, int with_udh, char *udh_data,
              int quick, int flash, char *messageids, char *smsc, int report, int validity, int part, int parts,
              int replace_msg,
...
// error_text: latest modem response. Might have a value even if return value is ok (when retry helped).
{
    ...
    *partstr = 0;
    ...
    make_pdu(to, text, textlen, alphabet, flash, report, with_udh, udh_data, DEVICE.mode, pdu, validity, replace_msg,
             system_msg, to_type, smsc);
    ...
    if (store_sent_pdu) {
        ...
        if (!outgoing_pdu_store) {
            ...
            outgoing_pdu_store = (char *) realloc((void *) outgoing_pdu_store,
                                                  strlen(outgoing_pdu_store) + strlen(title) + strlen(pdu) + 2);
    ...
    if (DEVICE.sending_disabled == 1 ||
        ...
        writelogfile(LOG_NOTICE, 0, "SMS sent, Message_id: %s, To: %s, sending time %i sec.", messageids, to,
                     time(0) - start_time);
        ...
        while (1) {
            ...
            if (strstr(answer, "OK")) {
                ...
                *answer = 0;
                ...
                writelogfile(LOG_NOTICE, 0, "SMS sent%s, Message_id: %s%s, To: %s, sending time %i sec.%s", partstr,
                             tmpid, replacestr, to, time(0) - start_time, answer);
                ...
                if (result) {
                    ...
                    writelogfile0(LOG_WARNING, 1,
                                  tb_sprintf("Sending SMS%s to %s failed, trying time %i sec. Retries: %i.", partstr,
                                             to, time(0) - start_time, retries - 1));
                    alarm_handler0(LOG_WARNING, tb);
                    flush_smart_logging();
                    return result;
                }
...
//    if (*udh_data && split)
            ...
            textlen = (int)iconv_utf2ucs(text, textlen, sizeof(text));
                ...
                if (parts > 1)
                    writelogfile(LOG_INFO, 0, "Splitting this message into %i parts of max %i characters%s.",
                                 parts, (alphabet == 2) ? eachpartlen / 2 : eachpartlen,
                                 (alphabet == 2) ? " (unicode)" : "");
            }
        }
        else {
            ...
            if (textlen > maxpartlen) {
                if (system_msg) {
                    ...
                    writelogfile0(LOG_NOTICE, 0,
                                  tb_sprintf("The file %s has too long data for system message: %i (max: %i).",
                                             filename, textlen, maxpartlen));
                    alarm_handler0(LOG_NOTICE, tb);
                    ...
                    writelogfile0(LOG_NOTICE, 0,
                                  tb_sprintf(
                                          "The file %s has too long data for single part (Autosplit: 0) sending: %i.",
                                          filename, textlen));
                    alarm_handler0(LOG_NOTICE, tb);
    ...
    for (part = 0; part < parts; part++) {
        ...
        if (part == 0 && voicecall == 1) {
            ...
            if (DEVICE.modem_disabled == 1) {
            ...
            else if (initialize_modem_sending("")) {
                ...
                if (DEVICE.voicecall_cpas || DEVICE.voicecall_clcc) {
                    ...
                    for (; ;) {
                        ...
                        put_command((DEVICE.voicecall_cpas) ? "AT+CPAS\r" : "AT+CLCC\r", answer, sizeof(answer), 24,
                                    expect);
                    }
                }
                else {
                ...
                if (strstr(answer, "OK")) {
                    ...
                    while (*ptr) {
                        ...
                        if (DEVICE.voicecall_vts_list)
                            ...
                            sprintf(strchr(command, 0), "+VTS=%s%c%s%c", quotation_mark, *ptr, quotation_mark,
                                    end_char);
                        ptr++;
                        tones++;
                    }
            ...
            i = send_part((from[0]) ? from : DEVICE.number, to, part_text, part_text_length, alphabet, with_udh,
                          udh_data,
                          *quick, flash, messageids, smsc, report, validity, part, parts, replace_msg, system_msg,
                          to_type, error_text);
    ...
    if (success < 0) {
        ...
        *add_headers = 0;
        ...
        if (eventhandler[0] || DEVICE.eventhandler[0]) {
            if (DEVICE.eventhandler[0])
                snprintf(cmdline, sizeof(cmdline), "%s FAILED %s %s", DEVICE.eventhandler,
                         (d_failed[0] == 0 || file_is_empty) ? filename : newfilename, messageids);
            else
                snprintf(cmdline, sizeof(cmdline), "%s FAILED %s %s", eventhandler,
                         (d_failed[0] == 0 || file_is_empty) ? filename : newfilename, messageids);
            exec_system(cmdline, EXEC_EVENTHANDLER);
        }
        ...
        if (success == -1) {
            ...
            if (*errorcounter >= blockafter) {
                writelogfile0(LOG_CRIT, 1,
                              tb_sprintf("Fatal error: sending failed %i times. Blocking %i sec.", blockafter,
                                         blocktime));
                alarm_handler0(LOG_CRIT, tb);
        ...
        *add_headers = 0;
        ...
        if (eventhandler[0] || DEVICE.eventhandler[0]) {
            ...
            if (DEVICE.eventhandler[0])
                snprintf(cmdline, sizeof(cmdline), "%s SENT %s %s", DEVICE.eventhandler,
                         (d_sent[0] == 0) ? filename : newfilename, messageids);
            else
                snprintf(cmdline, sizeof(cmdline), "%s SENT %s %s", eventhandler,
                         (d_sent[0] == 0) ? filename : newfilename, messageids);
            exec_system(cmdline, EXEC_EVENTHANDLER);
        }
...
#undef USSDOK
            ...
            if (is_ussd && DEVICE.eventhandler_ussd[0]) {
                ...
                n = snprintf(buffer, sizeof(buffer), "%s USSD %s %s %s \"", DEVICE.eventhandler_ussd, tmp_filename,
                             DEVICE.name, te_charset);
            ...
            if (DEVICE.dev_rr_logfile[0]) {
                while (log_retry-- > 0) {
                    ...
                    if ((fd = open(DEVICE.dev_rr_logfile, O_APPEND | O_WRONLY | O_CREAT, 0640)) >= 0) {
                        snprintf(buffer, sizeof(buffer), "%s,%i, %s: CMD: %s: %s\n", timestamp, DEVICE.dev_rr_loglevel,
                                 DEVICE.name, command, answer);
...
int run_rr() {
    ...
    if (DEVICE.dev_rr_post_run[0]) {
        ...
        // 3.1.9: added devicename.
        snprintf(cmdline, sizeof(cmdline), "%s %s \"%s\" %s", DEVICE.dev_rr_post_run, p, DEVICE.dev_rr_statfile,
                 DEVICE.name);
        i = exec_system(cmdline, EXEC_RR_POST_MODEM);
        if (i) {
            if (DEVICE.dev_rr[0] == 0 || strcmp(DEVICE.dev_rr, DEVICE.dev_rr_post_run))
                writelogfile0(LOG_ERR, 0,
                              tb_sprintf("Regular_run_post_run %s %s returned %i", DEVICE.dev_rr_post_run, p, i));
            else
    ...
    if ((fp = fopen(con_filename, "r"))) {
        ...
        if (!(fptmp = fopen(tmp_filename, "w"))) {
            writelogfile0(LOG_WARNING, 0,
                          tb_sprintf("Concatenation storage handling aborted, creating %s failed", tmp_filename));
            alarm_handler0(LOG_WARNING, tb);
            fclose(fp);
            return;
        }
...
int send_startstring() {
    ...
    if (DEVICE.startstring[0]) {
        ...
        do {
            ...
            if (strstr(answer, "ERROR")) if (retries < 2)
                t_sleep(1);
        }
    ...
    if (*suspend_filename) {
        ...
        while ((fp = fopen(suspend_filename, "r"))) {
            ...
            if (suspended && (!found || break_suspend)) {
                ...
                if (modem_was_open) if (try_openmodem()) if (initialized) if (send_startstring())
                    return 1;
            }
...
void devicespooler() {
    ...
    *smsd_debug = 0;
    ...
    if (DEVICE.report == 1 && !DEVICE.incoming && DEVICE.outgoing) {
        writelogfile0(LOG_WARNING, 0,
                      tb_sprintf("Cannot receive status reports because receiving is disabled on modem %s",
                                 DEVICE.name));
        alarm_handler0(LOG_WARNING, tb);
        DEVICE.report = 0;
    }
    ...
    if (DEVICE.communication_delay > 0)
        writelogfile(LOG_NOTICE, 0, "Using communication_delay between new commands %i milliseconds.",
    ...
    if (DEVICE.incoming) {
        ...
        if (!(check_memory_buffer = (char *) malloc(check_memory_buffer_size))) {
            writelogfile0(LOG_CRIT, 1, tb_sprintf("Did not get memory for check_memory_buffer (%i). Stopping.",
    ...
    while (terminate == 0) /* endless loop */
    {
        ...
        while (!terminate && DEVICE.outgoing) {
            ...
            if (DEVICE.message_limit > 0) if (message_count >= DEVICE.message_limit)
                break;
            ...
            if (i > 0) {
                ...
                if (max_continuous_sending > 0) {
                    if (time(0) >= started_sending + max_continuous_sending) {
                        writelogfile0(LOG_DEBUG, 0,
                                      "Max continuous sending time reached, will do other tasks and then continue.");
                        workless = 0;
                        ...
                        if (continuous_sent) {
                            ...
                            writelogfile(LOG_INFO, 0,
                                         "Sent %d messages in %d sec. Average time for one message: %.1f sec.",
